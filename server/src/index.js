...
// Runs before every connection; rejects sockets with missing/invalid tokens.
io.use((socket, next) => {
  try {
    // Client sends token via auth object (preferred) or query param (fallback)
    const token =
    ...
    socket.user = decoded; // available in all event handlers as socket.user
    next();
  } catch (err) {
